...
class FileStorageLoader extends Utilitie
{
    ...
    function upload()
    {
        ...
        if (move_uploaded_file($this->file['tmp_name'][$this->index], $destination . $name)) :
            RecordLoader::history("New image uploaded successfully.", 1);

            // $uploadedFilePath = $destination . $name;
            // $outputFilePath = $destination . $name . '.webp';

            // if ($this->convertAndResizeToWebP($uploadedFilePath, $outputFilePath, $destination)) :
            //     $uploaded = DS . $this->location . $name . '.webp';
            // else :
